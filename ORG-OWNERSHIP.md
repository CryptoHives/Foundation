
*The Keepers of the CryptoHives* is a collective authorship label representing the
contributors to the *CryptoHives Labs* projects.

All rights are assigned to the *CryptoHives Labs*.

---

## 📜 Contributor License Agreement (CLA)

All contributors must accept the *CryptoHives Labs* Contributor License
Agreement (CLA), which grants the Labs the right to use, modify, and
relicense contributions under the same terms as the rest of the project.

The CLA ensures consistent copyright ownership and unified licensing across all
subprojects.

---

## ⚙️ Organizational Control

The following assets are under the control of the Labs:

- Trademarks (reg. pending): "CryptoHives Labs"
- GitHub Organization: [github.com/CryptoHives](https://github.com/CryptoHives)
- Nuget Organization: [nuget.org/CryptoHives](https://nuget.org/CryptoHives)
- Primary Domains: [cryptohives.org](https://cryptohives.org), [cryptohives.de](https://cryptohives.de)
- All registered or pending ownerships are assigned to: Martin Regen, Germany

---

## 💬 Contact

**Martin Regen**  
Founder and Keeper — CryptoHives Labs  
📧 info@cryptohives.org
🌐 https://cryptohives.org

---

*This document declares public ownership and governance of the CryptoHives Labs
and its affiliated projects for transparency and legal clarity.*
