...
public sealed class AesCcm192 : AesCcm
{
    ...
    public AesCcm192(ReadOnlySpan<byte> key) : this(AesCore.AesDefault, key)
    {
    }
    ...
    /// <param name="key">The 24-byte key.</param>
    internal AesCcm192(SimdSupport simdSupport, ReadOnlySpan<byte> key) : base(simdSupport, key)
    {
    ...
    public static AesCcm192 Create(ReadOnlySpan<byte> key) => new(key);
    ...
    /// <param name="key">The 24-byte key.</param>
    /// <returns>A new AES-192-CCM instance.</returns>
    internal static AesCcm192 Create(SimdSupport simdSupport, ReadOnlySpan<byte> key) => new(simdSupport, key);
}
