...
{
    /// <summary>
    /// The number of blocks processed in parallel by the NEON 4-wide path.
    /// </summary>
    internal const int NeonParallelBlocks = 4;

    // Precomputed T-tables: LTn[x] = L(SBox[x] << (24 - 8*n)).
    //
    // The T transform (τ followed by L) decomposes by byte position because L is
    // linear over GF(2):
    //
    //   T(a) = L(τ(a))
    //        = L(SBox[b3]<<24  ⊕  SBox[b2]<<16  ⊕  SBox[b1]<<8  ⊕  SBox[b0])
    //        = LT0[b3]  ⊕  LT1[b2]  ⊕  LT2[b1]  ⊕  LT3[b0]
    //
    // This reduces 4 S-box lookups + 5 rotations + 5 XORs per T call to
    // 4 table lookups + 3 XORs, giving a 2–3× throughput improvement per round.
    // Total memory cost: 4 × 256 × 4 = 4 KiB (fits in typical L1 data cache).

    private static ReadOnlySpan<uint> LT0 =>
    [
    ...
    private static ReadOnlySpan<uint> LT1 =>
    [
    ...
    private static ReadOnlySpan<uint> LT2 =>
    [
    ...
    private static ReadOnlySpan<uint> LT3 =>
    [
    ...
    /// <summary>
    /// Applies the combined τ (S-box substitution) and L (linear) transforms using
    /// precomputed T-tables, eliminating per-round rotations.
    /// </summary>
    /// <remarks>
    /// Because L is XOR-linear, T(a) decomposes per byte position:
    /// T(a) = LT0[a>>24] ⊕ LT1[(a>>16)&amp;0xFF] ⊕ LT2[(a>>8)&amp;0xFF] ⊕ LT3[a&amp;0xFF].
    /// </remarks>
    [MethodImpl(MethodImplOptionsEx.HotPath)]
    private static uint TTable(uint a)
    {
        ref uint t0 = ref MemoryMarshal.GetReference(LT0);
        ref uint t1 = ref MemoryMarshal.GetReference(LT1);
        ref uint t2 = ref MemoryMarshal.GetReference(LT2);
        ref uint t3 = ref MemoryMarshal.GetReference(LT3);
        return Unsafe.Add(ref t0, a >> 24)
             ^ Unsafe.Add(ref t1, (a >> 16) & 0xFF)
             ^ Unsafe.Add(ref t2, (a >> 8) & 0xFF)
             ^ Unsafe.Add(ref t3, a & 0xFF);
    }

    /// <summary>
    /// Encrypts a single 16-byte block using T-table optimized round function.
    /// </summary>
    [MethodImpl(MethodImplOptionsEx.OptimizedLoop)]
    private static void EncryptBlockNeon(ReadOnlySpan<byte> input, Span<byte> output, uint* roundKeys)
    {
        ...
        {
            uint nx = x0 ^ TTable(x1 ^ x2 ^ x3 ^ roundKeys[i]);
    ...
    /// <summary>
    /// Decrypts a single 16-byte block using T-table optimized round function.
    /// </summary>
    [MethodImpl(MethodImplOptionsEx.OptimizedLoop)]
    private static void DecryptBlockNeon(ReadOnlySpan<byte> input, Span<byte> output, uint* roundKeys)
    {
        uint x0 = BinaryPrimitives.ReadUInt32BigEndian(input);
        uint x1 = BinaryPrimitives.ReadUInt32BigEndian(input[4..]);
        uint x2 = BinaryPrimitives.ReadUInt32BigEndian(input[8..]);
    ...
    /// <summary>
    /// Encrypts exactly four independent 16-byte blocks in parallel using ARM NEON
    /// wide XOR operations and T-table round function.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Each <see cref="Vector128{T}"/> of <see cref="uint"/> holds one 32-bit state word
    /// from each of the four blocks in its four lanes:
    /// </para>
    /// <code>
    ///   vX0 = [blk0.X0, blk1.X0, blk2.X0, blk3.X0]
    ///   vX1 = [blk0.X1, blk1.X1, blk2.X1, blk3.X1]
    /// </code>
    /// <para>
    /// The XOR combination <c>tmp = X1⊕X2⊕X3⊕rk</c> and the feedback XOR
    /// <c>nx = X0⊕T(tmp)</c> are computed with 128-bit NEON instructions,
    /// processing all four blocks simultaneously. The T-table lookup remains
    /// scalar per lane (NEON has no gather instruction).
    /// </para>
    /// </remarks>
    /// <param name="input">64-byte input buffer (4 consecutive 16-byte blocks).</param>
    /// <param name="output">64-byte output buffer.</param>
    /// <param name="roundKeys">Pointer to the 32 expanded round keys.</param>
    [MethodImpl(MethodImplOptionsEx.OptimizedLoop)]
    internal static void EncryptBlocks4Neon(ReadOnlySpan<byte> input, Span<byte> output, uint* roundKeys)
    {
        // Load x0..x3 from all four blocks. Each vector holds one word-position across
        // all four blocks: vXn = [blk0.Xn, blk1.Xn, blk2.Xn, blk3.Xn].
        var vX0 = Vector128.Create(
            BinaryPrimitives.ReadUInt32BigEndian(input),
            BinaryPrimitives.ReadUInt32BigEndian(input[16..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[32..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[48..]));
        var vX1 = Vector128.Create(
            BinaryPrimitives.ReadUInt32BigEndian(input[4..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[20..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[36..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[52..]));
        var vX2 = Vector128.Create(
            BinaryPrimitives.ReadUInt32BigEndian(input[8..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[24..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[40..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[56..]));
        var vX3 = Vector128.Create(
            BinaryPrimitives.ReadUInt32BigEndian(input[12..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[28..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[44..]),
            BinaryPrimitives.ReadUInt32BigEndian(input[60..]));

        for (int i = 0; i < Rounds; i++)
        {
            // XOR x1^x2^x3^rk for all four blocks simultaneously.
            var vRk = Vector128.Create(roundKeys[i]);
            var vTmp = AdvSimd.Xor(AdvSimd.Xor(vX1, vX2), AdvSimd.Xor(vX3, vRk));

            // T-table lookup for each lane (scalar – NEON has no gather).
            var vT = Vector128.Create(
                TTable(vTmp.GetElement(0)),
                TTable(vTmp.GetElement(1)),
                TTable(vTmp.GetElement(2)),
                TTable(vTmp.GetElement(3)));

            var vNx = AdvSimd.Xor(vX0, vT);
            vX0 = vX1; vX1 = vX2; vX2 = vX3; vX3 = vNx;
        }

        // Write four output blocks (reversed word order per SM4 spec).
        BinaryPrimitives.WriteUInt32BigEndian(output,       vX3.GetElement(0));
        BinaryPrimitives.WriteUInt32BigEndian(output[4..],  vX2.GetElement(0));
        BinaryPrimitives.WriteUInt32BigEndian(output[8..],  vX1.GetElement(0));
        BinaryPrimitives.WriteUInt32BigEndian(output[12..], vX0.GetElement(0));

        BinaryPrimitives.WriteUInt32BigEndian(output[16..], vX3.GetElement(1));
        BinaryPrimitives.WriteUInt32BigEndian(output[20..], vX2.GetElement(1));
        BinaryPrimitives.WriteUInt32BigEndian(output[24..], vX1.GetElement(1));
        BinaryPrimitives.WriteUInt32BigEndian(output[28..], vX0.GetElement(1));

        BinaryPrimitives.WriteUInt32BigEndian(output[32..], vX3.GetElement(2));
        BinaryPrimitives.WriteUInt32BigEndian(output[36..], vX2.GetElement(2));
        BinaryPrimitives.WriteUInt32BigEndian(output[40..], vX1.GetElement(2));
        BinaryPrimitives.WriteUInt32BigEndian(output[44..], vX0.GetElement(2));

        BinaryPrimitives.WriteUInt32BigEndian(output[48..], vX3.GetElement(3));
        BinaryPrimitives.WriteUInt32BigEndian(output[52..], vX2.GetElement(3));
        BinaryPrimitives.WriteUInt32BigEndian(output[56..], vX1.GetElement(3));
        BinaryPrimitives.WriteUInt32BigEndian(output[60..], vX0.GetElement(3));
    }

    /// <summary>
    /// Decrypts exactly four independent 16-byte blocks in parallel using ARM NEON.
    /// </summary>
    /// <param name="input">64-byte input buffer (4 consecutive 16-byte blocks).</param>
    /// <param name="output">64-byte output buffer.</param>
    /// <param name="roundKeys">Pointer to the 32 expanded round keys.</param>
    [MethodImpl(MethodImplOptionsEx.OptimizedLoop)]
    internal static void DecryptBlocks4Neon(ReadOnlySpan<byte> input, Span<byte> output, uint* roundKeys)
    {
        var vX0 = Vector128.Create(
            BinaryPrimitives.ReadUInt32BigEndian(input),
            BinaryPrimitives.ReadUInt32BigEndian(input[16..]),
