...
/// <remarks>
/// <para>
/// SM4 is a 128-bit block cipher with a 128-bit key, standardized by the Chinese
/// national standard GB/T 32907-2016. It uses 32 rounds of a Feistel-like nonlinear
/// transformation with big-endian byte order.
/// </para>
/// <para>
/// <b>Implementation notes:</b>
/// <list type="bullet">
///   <item><description>Uses a single 8×8 S-box (τ transform) applied to each byte</description></item>
///   <item><description>Round function: T = L ∘ τ (S-box substitution + linear transform)</description></item>
///   <item><description>Key expansion uses T' = L' ∘ τ with a different linear transform</description></item>
///   <item><description>Big-endian byte order throughout</description></item>
/// </list>
/// </para>
/// </remarks>
internal unsafe partial struct Sm4Core
{
    ...
    internal Sm4Core(ReadOnlySpan<byte> key)
    {
    ...
    /// </summary>
    /// <remarks>
    /// The S-box provides the nonlinear substitution defined in GB/T 32907-2016 §6.2.
    /// Each byte of a 32-bit word is independently substituted through this table.
    /// </remarks>
    ...
    /// <summary>
    /// System parameter FK used in key expansion.
    /// </summary>
    private static ReadOnlySpan<uint> FK =>
    [
        0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC,
    ];

    /// <summary>
    /// Constant key CK used in key expansion, computed as CK[i] byte j = (28*i + 7*j) mod 256.
    /// </summary>
    ...
    /// <summary>
    /// Encrypts a single 16-byte block using the SM4 algorithm.
    /// </summary>
    ...
    /// <summary>
    /// τ transform: applies the S-box substitution to each byte of a 32-bit word.
    /// </summary>
    /// <param name="a">The input word.</param>
    /// <returns>The substituted word.</returns>
    [MethodImpl(MethodImplOptionsEx.HotPath)]
    private static uint Tau(uint a)
    {
        return ((uint)SBox[(int)(a >> 24)] << 24) |
               ((uint)SBox[(int)((a >> 16) & 0xFF)] << 16) |
               ((uint)SBox[(int)((a >> 8) & 0xFF)] << 8) |
               SBox[(int)(a & 0xFF)];
    }

    /// <summary>
    /// L transform: linear transformation for encryption rounds.
    /// L(B) = B ⊕ (B &lt;&lt;&lt; 2) ⊕ (B &lt;&lt;&lt; 10) ⊕ (B &lt;&lt;&lt; 18) ⊕ (B &lt;&lt;&lt; 24).
    /// </summary>
    /// <param name="b">The input word.</param>
    /// <returns>The linearly transformed word.</returns>
    [MethodImpl(MethodImplOptionsEx.HotPath)]
    ...
    /// <summary>
    /// T transform: composition of τ (S-box substitution) and L (linear transform),
    /// computed via precomputed T-tables for performance.
    /// </summary>
    /// <param name="a">The input word.</param>
    ...
    /// <summary>
    /// L' transform: linear transformation for key expansion.
    /// L'(B) = B ⊕ (B &lt;&lt;&lt; 13) ⊕ (B &lt;&lt;&lt; 23).
    /// </summary>
    /// <param name="b">The input word.</param>
    /// <returns>The linearly transformed word.</returns>
    ...
    /// <summary>
    /// T' transform: composition of τ (S-box substitution) and L' (key expansion linear transform).
    /// </summary>
    /// <param name="a">The input word.</param>
    /// <returns>The transformed word.</returns>
    [MethodImpl(MethodImplOptionsEx.HotPath)]
