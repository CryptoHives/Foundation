...
/// <summary>
/// Provides X.509 CRL signature verification.
/// </summary>
public static class X509CrlValidator
{
    /// <summary>
    /// Verifies that a CRL's signature was created by the specified issuer certificate.
    /// </summary>
    /// <param name="crl">The CRL to verify.</param>
    /// <param name="issuerCert">The issuer certificate containing the signing public key.</param>
    /// <returns><c>true</c> if the signature is valid; otherwise, <c>false</c>.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="crl"/> or <paramref name="issuerCert"/> is <c>null</c>.</exception>
    public static bool VerifySignature(X509Crl crl, X509Certificate issuerCert)
    {
        if (crl is null) throw new ArgumentNullException(nameof(crl));
        if (issuerCert is null) throw new ArgumentNullException(nameof(issuerCert));

        return VerifyWithPublicKey(crl, issuerCert.SubjectPublicKeyInfoDer);
    }

    private static bool VerifyWithPublicKey(X509Crl crl, byte[] spkiDer)
    {
        string sigAlgOid = crl.SignatureAlgorithmOid;
        ...
        try
        {
            ...
            {
                "RSA" => VerifyRsa(crl.TbsCertListDer, crl.SignatureValue, spkiDer, hashAlg!, sigAlgOid),
                "ECDSA" => VerifyEcdsa(crl.TbsCertListDer, crl.SignatureValue, spkiDer, hashAlg!),
                "Ed25519" => VerifyEd25519(crl.TbsCertListDer, crl.SignatureValue, spkiDer),
                "Ed448" => VerifyEd448(crl.TbsCertListDer, crl.SignatureValue, spkiDer),
                _ => false,
            };
        }
        catch
