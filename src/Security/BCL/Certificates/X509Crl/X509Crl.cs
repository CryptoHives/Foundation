...
public class X509CRL : IX509CRL
{
    ...
    public X509CRL(IX509CRL crl) : this()
    {
        if (crl == null) throw new ArgumentNullException(nameof(crl));
    ...
    {
        _decoded = false;
        _thisUpdate = DateTimeOffset.MinValue;
        _nextUpdate = DateTimeOffset.MinValue;
        _revokedCertificates = new List<RevokedCertificate>();
        _crlExtensions = new X509ExtensionCollection();
        _issuerName = new X500DistinguishedName(Array.Empty<byte>());
        RawData = Array.Empty<byte>();
        _revokedSerialNumbersSet = new HashSet<byte[]>(ByteArrayEqualityComparer.Instance);
    }
    ...
    /// <inheritdoc/>
    public IList<RevokedCertificate> RevokedCertificates => _revokedCertificates.AsReadOnly();
    ...
    public bool IsRevoked(X509Certificate2 certificate)
    {
        ...
        EnsureRevokedSerialNumbersSet();
        byte[] serialNumber = certificate.GetSerialNumber();
        return _revokedSerialNumbersSet.Contains(serialNumber);
    }
    #endregion
    ...
    internal void DecodeCrl(byte[] tbs)
    {
        try
        {
            ...
            if (seqReader != null)
            {
                ...
                // thisUpdate
                _thisUpdate = ReadTime(seqReader, optional: false);

                // nextUpdate is OPTIONAL
                _nextUpdate = ReadTime(seqReader, optional: true);
    ...
    /// <summary>
    /// Ensures the revoked serial numbers HashSet is initialized for fast lookups.
    /// </summary>
    private void EnsureRevokedSerialNumbersSet()
    {
        if (_revokedSerialNumbersSet.Count == 0 && _revokedCertificates.Count > 0)
        {
            for (int i = 0; i < _revokedCertificates.Count; i++)
            {
                _revokedSerialNumbersSet.Add(_revokedCertificates[i].UserCertificate);
            }
        }
    }
    #endregion
    ...
    private HashSet<byte[]> _revokedSerialNumbersSet;
    #endregion

    #region Nested Types
    /// <summary>
    /// Equality comparer for byte arrays used in HashSet lookups.
    /// </summary>
