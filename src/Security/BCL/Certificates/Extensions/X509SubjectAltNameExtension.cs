...
public class X509SubjectAltNameExtension : X509Extension
{
    ...
    private byte[] Encode()
    {
        ...
        EncodeGeneralNames(sanBuilder, _domainNames);
        EncodeGeneralNames(sanBuilder, _ipAddresses);
    ...
    private void Decode(byte[] data)
    {
        ...
        if (oidValue == SubjectAltNameOid ||
            oidValue == SubjectAltName2Oid)
        {
            try
            {
                ...
                AsnReader sanReader = dataReader.ReadSequence();
                ...
                while (sanReader.HasData)
                {
                    Asn1Tag peekTag = sanReader.PeekTag();
                    if (peekTag == uriTag)
                    {
                        string uri = sanReader.ReadCharacterString(UniversalTagNumber.IA5String, uriTag);
                        uris.Add(uri);
                    }
                    else if (peekTag == dnsTag)
                    {
                        string dnsName = sanReader.ReadCharacterString(UniversalTagNumber.IA5String, dnsTag);
                        domainNames.Add(dnsName);
                    }
                    else if (peekTag == ipTag)
                    {
                        byte[] ip = sanReader.ReadOctetString(ipTag);
                        ipAddresses.Add(IPAddressToString(ip));
                    }
                    else  // skip over
                    {
                        sanReader.ReadEncodedValue();
                    }
                }
                sanReader.ThrowIfNotEmpty();
                _uris = uris;
    ...
    {
        _uris.Add(applicationUri);
        foreach (string generalName in generalNames)
        {
            switch (Uri.CheckHostName(generalName))
            {
                case UriHostNameType.Dns:
                    _domainNames.Add(generalName);
                    break;
                case UriHostNameType.IPv4:
                case UriHostNameType.IPv6:
                    _ipAddresses.Add(generalName);
                    break;
