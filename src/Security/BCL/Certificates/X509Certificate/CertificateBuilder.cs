...
public class CertificateBuilder : CertificateBuilderBase
{
    ...
    public override X509Certificate2 CreateForRSA()
    {
        ...
        var serialNumberAsSpan = _serialNumber.AsSpan();
        serialNumberAsSpan.Reverse();
        var serialNumber = serialNumberAsSpan.ToArray();
        if (IssuerCAKeyCert != null)
        {
            using RSA? rsaIssuerKey = IssuerCAKeyCert.GetRSAPrivateKey();
            if (rsaIssuerKey == null)
            {
                throw new CryptographicException("Issuer certificate does not contain an RSA private key.");
            }
        ...
        else
        {
            if (rsaKeyPair == null)
            {
                throw new InvalidOperationException("RSA key pair was not created.");
            }
            signedCert = request.Create(
                SubjectName,
    ...
    public override X509Certificate2 CreateForECDsa()
    {
        ...
        if (publicKey == null)
        {
            if (_curve == null)
            {
                throw new InvalidOperationException("ECCurve is required when no public key is provided.");
            }
            key = ECDsa.Create(_curve.Value);
            publicKey = key;
        }
        ...
        if (IssuerCAKeyCert != null)
        {
            ...
            if (issuerKey == null)
            {
                throw new CryptographicException("Issuer certificate does not contain an ECDsa private key.");
            }
        ...
        else
        {
            if (key == null)
            {
                throw new InvalidOperationException("ECDsa key was not created.");
            }
            return request.Create(
                SubjectName,
    ...
    private void CreateX509Extensions(CertificateRequest request, bool forECDsa)
    {
        ...
        if (_extensions.FindExtension<X509AuthorityKeyIdentifierExtension>() == null)
        {
            ...
            {
                string skiValue = ski.SubjectKeyIdentifier ?? throw new InvalidOperationException("SubjectKeyIdentifier is null.");
                authorityKeyIdentifier = new CryptoHives.Foundation.Security.Bcl.Certificates.X509AuthorityKeyIdentifierExtension(
