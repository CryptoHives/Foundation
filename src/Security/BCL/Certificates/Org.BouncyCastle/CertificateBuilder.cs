...
using CryptoHives.Foundation.Security.Bcl.Certificates.BouncyCastle;
...
public class CertificateBuilder : CertificateBuilderBase
{
        ...
        )
    {
        ...
        if (san == null)
        {
            throw new CryptographicException("Certificate does not contain a Subject Alternative Name extension.");
        }
        ...
        if (alternateName != null)
        {
            ...
            foreach (string name in alternateName.DomainNames)
            {
                ...
                {
                    if (domainNames[i].Equals(name, StringComparison.OrdinalIgnoreCase))
                    {
                        found = true;
                        break;
                    }
            ...
            foreach (string ipAddress in alternateName.IPAddresses)
            {
                ...
                for (int i = 0; i < domainNames.Count; i++)
                {
                    if (domainNames[i].Equals(ipAddress, StringComparison.OrdinalIgnoreCase))
                    {
                        found = true;
                        break;
                    }
    ...
    private void CreateMandatoryFields(X509V3CertificateGenerator cg)
    {
        ...
        byte[] serialNumber = serialNumberAsSpan.ToArray();
        cg.SetSerialNumber(new BigInteger(1, serialNumber));
    }
    ...
    private void CreateExtensions(X509V3CertificateGenerator cg, AsymmetricKeyParameter subjectPublicKey)
    {
        ...
        else
        {
            ...
            serialNumberAsSpan.Reverse();
            byte[] serialNumber = serialNumberAsSpan.ToArray();
            issuerSerialNumber = new BigInteger(1, serialNumber);
        }
    ...
    private X509Certificate2 CreateForRSAWithPublicKey(ISignatureFactory? signatureFactory = null)
    {
        ...
        if (_rsaPublicKey == null)
        {
            throw new InvalidOperationException("Need a public key for the certificate.");
        }
        ...
        if (signatureFactory == null)
        {
            if (IssuerCAKeyCert == null)
            {
                throw new InvalidOperationException("Issuer certificate is required for signing.");
            }
