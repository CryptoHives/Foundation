...
public static class X509PfxUtils
{
    ...
    /// <summary>
    /// Thread-safe random number generator for older frameworks.
    /// </summary>
    ...
    /// <summary>
    /// Fills a byte array with cryptographically secure random bytes in a thread-safe manner.
    /// </summary>
    /// <param name="data">The byte array to fill with random bytes.</param>
    private static void FillRandomBytes(byte[] data)
    {
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP2_1_OR_GREATER || NET5_0_OR_GREATER
    ...
    private static X509KeyUsageFlags GetKeyUsage(X509Certificate2 cert)
    {
        ...
        foreach (X509Extension ext in cert.Extensions)
        {
            ...
            {
                allFlags |= keyUsageExt.KeyUsages;
            }
        }
        return allFlags;
    }
    ...
    internal static bool VerifyRSAKeyPairCrypt(RSA rsaPublicKey, RSA rsaPrivateKey)
    {
        ...
        {
            // Avoid LINQ SequenceEqual - use span-based comparison
            return testBlock.AsSpan().SequenceEqual(decryptedBlock.AsSpan());
        }
