...
public sealed class CrlBuilder : IX509CRL
{
    ...
    private CrlBuilder(IX509CRL crl)
        : this(crl.IssuerName, crl.HashAlgorithmName)
    {
    ...
    public CrlBuilder AddRevokedSerialNumbers(string[] serialNumbers, CRLReason crlReason = CRLReason.Unspecified)
    {
        ...
        {
            _revokedCertificates.Add(new RevokedCertificate(serialNumbers[i], crlReason));
        }
        return this;
    }
    ...
    public IX509CRL CreateForECDsa(X509Certificate2 issuerCertificate)
    {
        ...
        if (ecdsa == null)
        {
            throw new CryptographicException("Certificate does not contain an ECDsa private key.");
        }
    ...
    private readonly X509ExtensionCollection _crlExtensions;
    #endregion
}
