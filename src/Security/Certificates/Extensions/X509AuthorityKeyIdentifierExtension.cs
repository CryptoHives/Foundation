...
public class X509AuthorityKeyIdentifierExtension : X509Extension
{
    ...
    public X509AuthorityKeyIdentifierExtension(byte[] subjectKeyIdentifier)
    {
        ...
        _keyIdentifier = subjectKeyIdentifier;
    ...
    public override string Format(bool multiLine)
    {
        ...
        if (_keyIdentifier != null && _keyIdentifier.Length > 0)
        {
            if (buffer.Length > 0)
            {
                if (multiLine)
            ...
            buffer.Append('=');
            buffer.Append(_keyIdentifier.ToHexString());
        }
        ...
        if (_issuer != null)
        {
            ...
            buffer.Append('=');
            buffer.Append(_issuer.Format(true));
        }

        if (_serialNumber != null && _serialNumber.Length > 0)
        {
            if (buffer.Length > 0)
            {
                if (!multiLine)
            ...
            buffer.Append('=');
            buffer.Append(_serialNumber.ToHexString(true));
        }
    ...
    /// </summary>
    public string KeyIdentifier => _keyIdentifier.ToHexString();

    /// <summary>
    /// The identifier for the key as a byte array.
    /// </summary>
    public byte[]? GetKeyIdentifier() => _keyIdentifier;

    /// <summary>
    /// A list of distinguished names for the issuer.
    /// </summary>
    public X500DistinguishedName? Issuer => _issuer;
    ...
    /// </summary>
    public byte[]? GetSerialNumber() => _serialNumber;
    #endregion
    ...
    private byte[] Encode()
    {
        ...
        if (_keyIdentifier != null)
        {
            ...
            writer.WriteOctetString(_keyIdentifier, keyIdTag);
        }
        ...
        if (_issuer != null)
        {
            ...
            writer.WriteEncodedValue(_issuer.RawData);
        ...
        if (_serialNumber != null)
        {
            ...
            var issuerSerial = new BigInteger(_serialNumber);
    ...
    {
        string? oidValue = base.Oid?.Value;
        if (oidValue == AuthorityKeyIdentifierOid ||
            oidValue == AuthorityKeyIdentifier2Oid)
        {
            try
            {
                ...
                while (akiReader.HasData)
                {
                    ...
                    if (peekTag == keyIdTag)
                    {
                        _keyIdentifier = akiReader.ReadOctetString(keyIdTag);
                        continue;
                    }
                    ...
                    if (peekTag == dnameSequencyTag)
                    {
                        ...
                        var directoryNameTag = new Asn1Tag(TagClass.ContextSpecific, 4, true);
                        AsnReader dirNameReader = issuerReader.ReadSequence(directoryNameTag);
                        _issuer = new X500DistinguishedName(dirNameReader.ReadEncodedValue().ToArray());
                    ...
                    {
                        _serialNumber = akiReader.ReadInteger(serialNumberTag).ToByteArray();
                        continue;
                    }
