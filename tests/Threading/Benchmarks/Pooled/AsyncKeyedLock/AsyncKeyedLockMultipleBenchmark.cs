...
public class AsyncKeyedLockMultipleBenchmark : AsyncKeyedLockBaseBenchmark
{
    ...
    private ValueTask<global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser>[]? _stripedHandle;
    ...
    private ValueTask<global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser?>[]? _stripedTimedHandle;
    ...
    {
        SetUpKeys();
        _stripedHandle = new ValueTask<global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser>[Iterations];
    }
    ...
    public async Task LockUnlockStripedMultipleAsync(CancellationType cancellationType)
    {
        for (int k = 0; k < KeyCount; k++)
        {
            global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser outer =
    ...
    {
        SetUpKeys();
        _stripedTimedHandle = new ValueTask<global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser?>[Iterations];
    }
    ...
    public async Task LockUnlockStripedTimedMultipleAsync(CancellationType cancellationType)
    {
        for (int k = 0; k < KeyCount; k++)
        {
            global::AsyncKeyedLock.StripedAsyncKeyedLockReleaser? outer = await _lockStriped
                .LockOrNullAsync(_keys![k], cancellationType.Timeout, cancellationType.CancellationToken)
                .ConfigureAwait(false);
