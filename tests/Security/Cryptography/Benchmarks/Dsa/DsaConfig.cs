...
/// <summary>
/// BenchmarkDotNet configuration for signature benchmarks.
/// </summary>
/// <remarks>
/// Mirrors <c>KemConfig</c>: signature operations are fixed-size, so there is no data-size
/// axis. Results group by parameter set (ML-DSA-44/65/87) and then by operation, which puts
/// the implementations of the same operation next to each other — the comparison the suite
/// exists to make.
/// </remarks>
public class DsaConfig : ManualConfig
{
    /// <summary>
    /// Initializes a new instance of the <see cref="DsaConfig"/> class.
    /// </summary>
    public DsaConfig()
    {
        ...
        HideColumns("Method", "TestDsaAlgorithm", "Code Size");
    ...
    private class DescriptionColumn : IColumn
    {
        ...
        public string GetValue(Summary summary, BenchmarkCase benchmarkCase)
        {
            ...
            var dsaAlgorithm = benchmarkCase.Parameters["TestDsaAlgorithm"] as DsaAlgorithmType;

            if (dsaAlgorithm != null)
            {
                return $"{method} · {dsaAlgorithm.Category} · {GetImplementationType(dsaAlgorithm.Name)}";
            }

            // Benchmarks without a TestDsaAlgorithm parameter fall back to the bare method
            // description, as the KEM internals suite does.
            return method;
        }
    ...
    private class CategoryThenOperationOrderer : IOrderer
    {
        ...
        {
            var dsaAlgorithm = benchmarkCase.Parameters["TestDsaAlgorithm"] as DsaAlgorithmType;
            return dsaAlgorithm?.Name.EndsWith("(CryptoHives)", StringComparison.Ordinal) == true
                ? "CryptoHives"
                : null;
        }
        ...
        /// <summary>
        /// Orders the operations the way one reads them — generate a key, sign, verify —
        /// rather than alphabetically, which would lead with KeyGen anyway but put Sign after
        /// Verify.
        /// </summary>
        /// <remarks>
        ...
        /// algorithm parameter or a plain string parameter.
        /// </summary>
        private static string GetParameterSet(BenchmarkCase benchmark)
        {
            if (benchmark.Parameters["TestDsaAlgorithm"] is DsaAlgorithmType dsaAlgorithm)
            {
                return dsaAlgorithm.Category;
            }
