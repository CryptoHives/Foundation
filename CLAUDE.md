...
- Encrypted PKCS#8 **writes PBES2 only** (PBKDF2 + AES-CBC) but **reads the legacy PKCS#12 schemes too**, since that is what older OpenSSL and Windows wrote. Those need TripleDES/RC2, which this library does not implement, so that path alone delegates to the platform — `PbeEncryptionAlgorithm.TripleDes3KeyPkcs12` is import-only
- **`PbeParameters`/`PbeEncryptionAlgorithm` ship as a shim** in `shared/Pbe/` for net462/net472/netstandard2.0, and are the only types this library puts in the `System.Security.Cryptography` namespace — they appear in public signatures, so an internal CryptoHives-namespace shim cannot work. Collides only with `Microsoft.Bcl.Cryptography` (measured; `System.Security.Cryptography.Pkcs` does not define them downlevel)
