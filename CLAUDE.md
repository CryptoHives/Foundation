...
- **`CryptographicOperations.ZeroMemory` is public** (`src/Security/Cryptography/shared/`, our namespace, all six TFMs) and is **the** way to erase a secret buffer - in library code, in tests, and in every doc sample. Never reach for `Array.Clear` or `Span<T>.Clear()` on something secret: clearing a buffer nothing reads again is a dead store a compiler may delete, which is exactly what the `NoInlining | NoOptimization` on these methods prevents. We ship our own because the in-box type does not exist below netstandard2.1 and has **no `char` overload on any .NET version** - so a password or a PEM document in `char` storage has no in-box eraser at all. It collides by name with `System.Security.Cryptography.CryptographicOperations` (CS0104, verified) exactly as `PbeEncryptionAlgorithm` does; the fix is a consumer-side `using Bcl = System.Security.Cryptography;`. **`FixedTimeEquals` is also public**, for the same downlevel reason and because `mac-algorithms.md` already told readers to call it - as written that sample did not compile on net462/net472/netstandard2.0, where the in-box type does not exist (verified). Use it, not `SequenceEqual`, for any MAC or tag check. `FixedTimeEqualsMask` stays **internal**: it is the branchless-select primitive for ML-KEM implicit rejection, not a general "are these equal". **It is deliberately not a polyfill**: shipping it as one means declaring `System.Security.Cryptography.CryptographicOperations` downlevel, where **`Microsoft.Bcl.Cryptography` already declares it** (verified in the package) - a hard CS0433 rather than a CS0104 an alias fixes - and forwarding to the in-box type on net8/net10 would drop the `char` overloads. Decided; don't re-open it. Tracking issue for the internal `Array.Clear` sweep: [#294](https://github.com/CryptoHives/Foundation/issues/294)
- **Erasable memory only, on the key-format surface.** No public member takes a password, or returns a plaintext private key, as a `string` — a `string` cannot be overwritten, so a secret in one lives on the heap until the collector reuses the memory. Passwords are `ReadOnlySpan<char>`/`ReadOnlySpan<byte>`; the plaintext PEM export is `TryExportPkcs8PrivateKeyPem(Span<char>, out int)` sized by `GetPkcs8PrivateKeyPemSize()`, with **no allocating counterpart**; `ImportFromPem` takes only a span. `string` survives on exactly two members, whose content is public by construction: `ExportSubjectPublicKeyInfoPem` and `ExportEncryptedPkcs8PrivateKeyPem`. `ErasableMemoryTests` enforces this by reflection — adding a `string` password overload for convenience will fail the build's test run. The 21 members this replaced are **not deleted**: they are in the tree under `#if OBSOLETE_SECRET_AS_STRING_API`, which no shipping build defines (opt in with `-p:EnableObsoleteSecretAsStringApi=true`), so a reviewer can diff them against what replaced them. `docfx/packages/security/cryptography/erasable-memory.md` is the porting table. One residual is documented rather than fixed: `AsnWriter` keeps an unclearable internal copy of everything written to it, and closing that would mean hand-rolling the two private-key DER structures
